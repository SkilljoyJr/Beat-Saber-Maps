...
Shader "You/IntroSkybox"
{
    Properties
    {
        ...
        _BFC ("BFC", Range (0,10)) = 0
        _Swirl ("Swirl", Range (0,10)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                col *= 2 * pos.z * (1 + sin(1 * 60) * 0.1 * _Flutter);

                float f = 1 - length(pos.xy);
                {
                    float twist = f * 3 * sin(1 * 0.1) + 10 - pos.z * 5;
                    float a = sin(rawAngle * 2 + sin(1 * 0.1) + twist);
                    a -= pos.z;
                ...
                // Swirls
                float dist = sin(length(pos.xy*_Swirl) * 1 + 1 * 1 + rawAngle * 6);
                float pattern = cos(rawAngle * 5 + dist *2) - cos(rawAngle * 4);
                pattern = pattern * 0.5 + 0.5;
                // pattern *= pos.z * 2;
                pattern *= saturate(99 - pos.z * 1.1);
