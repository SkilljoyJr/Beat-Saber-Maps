Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/82d72b14b5e69ce49a34325073be8264"
{
	Properties
	{
 ...
 #define PROP_TRUCHETTEX 
 #define PROP_TRUCHETMASK 
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionTex__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 1.0);
				}
				...
				if(1.0)
				{
					emissionColor0__1 = hueShift(emissionColor0__1, frac(_EmissionHueShift1 + 0.0 * _Time.x), 0.0, 1.0);
					emissionColor0__1 = lerp(emissionColor0__1, dot(emissionColor0__1, float3(0.3, 0.59, 0.11)), - (4.83));
				}
				#if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0__1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0,0)) + _Time.x * float4(0.05,-0.05,0,0))[0.0];
				#else
				...
				if (0.0 > 0)
				{
					emissionMask0__1 = maskBlend(emissionMask0__1, poiMods.globalMask[0.0 - 1], 7.0);
				}
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				for (uint glitterLayer = 0; glitterLayer < uint(4.0); glitterLayer++)
				{
					float2 st = (poiMesh.uv[0.0] + float4(0.005,0.01,0,0).xy * _Time.x) * 25.0;
					...
					half3 glitterColor = poiThemeColor(poiMods, float4(0.9622642,0.9622642,0.9622642,1).rgb, 0.0);
					float3 norm = lerp(poiMesh.normals[0], poiMesh.normals[1], 0.027);
					...
					switch(0.0)
					{
						...
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, 63.0, 0.0, glitterSpeedOffset);
						}
						else
						{
						...
						finalGlitter = lerp(0, 0.0 * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, 0.0), poiCam.viewDir)), 282.0), 0);
					...
					glitterMask *= float4(0.9622642,0.9622642,0.9622642,1).a;
					if (0.0 > 0)
					{
			...
			{
				float2 uv = poiMesh.uv[7.0] * _TruchetDensity + float4(0.1,0,0,0).xy * _Time.x + float4(0.1,0,0,0).zw;
				...
				#if defined(PROP_TRUCHETMASK) || !defined(OPTIMIZER_ENABLED)
				float maskTex = UNITY_SAMPLE_TEX2D_SAMPLER(_TruchetMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(0.2,0.2,0,0)) + _Time.x * float4(0,0,0,0))[1.0];
				#else
				float maskTex = 1;
				#endif
				...
				if (9.0 > 0)
				{
					maskTex = maskBlend(maskTex, poiMods.globalMask[9.0 - 1], 2.0);
				}
				mask *= maskTex;
				...
				#if defined(PROP_TRUCHETTEX) || !defined(OPTIMIZER_ENABLED)
				float4 TruchetTex = POI2D_SAMPLER_PANGRAD(_TruchetTex, _MainTex, poiUV(pathUV, float4(1,1,0,0)) + uvOffset, float4(1,2,0,0).xy, dx, dy);
				#else
				float4 TruchetTex = 1;
				#endif
				float3 truchetColor = poiThemeColor(poiMods, _TruchetColor.rgb, 0.0);
				poiFragData.emission += lerp(0, TruchetTex.rgb * truchetColor, min(mask, TruchetTex.a) * emission);
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, TruchetTex.rgb * truchetColor, 1.0), min(mask, TruchetTex.a) * 0.287);
				poiFragData.alpha *= lerp(1, min(mask, TruchetTex.a) * 0.287, 1.0);
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.002920729,0.002920729,0.002920729,1).rgb, 0.0);
