Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/efda69a99383a6146a9c3135e84546e3"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float2 uv = poiMesh.uv[7.0] * _TruchetDensity + float4(0.1,0.3,0,0).xy * _Time.x + float4(0.1,0.3,0,0).zw;
				...
				#if defined(PROP_TRUCHETMASK) || !defined(OPTIMIZER_ENABLED)
				float maskTex = UNITY_SAMPLE_TEX2D_SAMPLER(_TruchetMask, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0))[1.0];
				#else
				float maskTex = 1;
				#endif
				...
				#if defined(PROP_TRUCHETTEX) || !defined(OPTIMIZER_ENABLED)
				float4 TruchetTex = POI2D_SAMPLER_PANGRAD(_TruchetTex, _MainTex, poiUV(pathUV, float4(1,1,0,0)) + uvOffset, float4(1,0,0,0).xy, dx, dy);
				#else
				float4 TruchetTex = 1;
				#endif
				...
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, TruchetTex.rgb * truchetColor, 1.0), min(mask, TruchetTex.a) * 0.37);
				poiFragData.alpha *= lerp(1, min(mask, TruchetTex.a) * 0.37, 1.0);
			}
			#endif
