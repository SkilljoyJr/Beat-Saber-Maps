Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/efda69a99383a6146a9c3135e84546e3"
{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float2 uv = poiMesh.uv[6.0] * 10.0 + float4(1.23,0,0,0).xy * _Time.x + float4(1.23,0,0,0).zw;
				...
				mask = saturate((0.105 - abs(distance - 0.5)) / fwidth(abs(distance - 0.5)));
				mask *= smoothstep(0, 1 - 0.725, (0.105 - abs(distance - 0.5)));
				#if defined(PROP_TRUCHETMASK) || !defined(OPTIMIZER_ENABLED)
				float maskTex = UNITY_SAMPLE_TEX2D_SAMPLER(_TruchetMask, _MainTex, poiUV(poiMesh.uv[5.0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0))[1.0];
				#else
				float maskTex = 1;
				#endif
				...
				if (0.0 > 0)
				{
					maskTex = maskBlend(maskTex, poiMods.globalMask[0.0 - 1], 4.0);
				}
				mask *= maskTex;
				float angle = atan2(pathUV.x, pathUV.y);
				pathUV.x = checker * angle / 1.57;
				pathUV.y = (distance - (.5 - 0.105)) / (2 * 0.105);
				...
				float4 TruchetTex = 1;
				#endif
				float3 truchetColor = poiThemeColor(poiMods, float4(0.8760344,0.001722129,0.6894628,1).rgb, 0.0);
				poiFragData.emission += lerp(0, TruchetTex.rgb * truchetColor, min(mask, TruchetTex.a) * emission);
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, TruchetTex.rgb * truchetColor, 1.0), min(mask, TruchetTex.a) * 1.0);
